...
namespace TEST_CRUD.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<IEnumerable<GetOrderDto?>> GetByCustomerId(int customerid)
        {
            var orderList = appDbContext.Order.Where(od => od.IsDeleted != true);
            orderList = orderList.Where(od => od.Customer_Id == customerid);
        ...
        {
            var order = await appDbContext.Order.FirstOrDefaultAsync(od => od.Order_Number == id);
        ...
        public async Task<GetOrderDto> UpdateOrderStatus(string orderid, string status)
        {
            var order = await appDbContext.Order.FirstOrDefaultAsync(od => od.Order_Number == orderid);
            if (order == null)
            {
                return null;
            }
